...
export const parseDateTime = (str: Date | string, locale: Locale) => {
  ...
  const chronoLocale = chronoLocales.includes(localeCode as ChronoLocale)
    ? (localeCode as ChronoLocale)
    : chronoLocales.includes(localeCode.split('-')[0] as ChronoLocale)
      ? (localeCode.split('-')[0] as ChronoLocale)
      : 'en'

  return chrono[chronoLocale].parseDate(str)
}
...
export const formatDateTime = (
  ...
  new Date(datetime).toLocaleTimeString(locale?.code || 'en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
    hour: 'numeric',
  ...
  (
    {
      ...
      value,
      locale = localeEnUS,
      hour12 = localesUsings12HourTime.includes(locale?.code ?? 'en-US'),
      onValueChange,
      placeholder,
      disabled,
      ...dateTimeInputProps
    ...
    const safeLocale = locale || localeEnUS
...
const DateTimeLocalInput = ({
  ...
  const formateSelectedDate = React.useCallback(
    (date: Date | undefined) => {
      ...
      const parsedDateTime = parseDateTime(date, locale)
